...
>nul findstr /v "$" "%~nx0" && (
echo:
echo Error: 脚本存在 LF 行结尾问题，或者脚本末尾缺少空行。
echo:
...
echo 项目仅支持 Windows 7/8/8.1/10/11 及其对应服务器版本。
goto done2
)
...
echo WMI不在运行，正在终止
echo:
echo 请翻阅此页面获取帮助. %mas%IAS-Help#troubleshoot
goto done2
)
...
echo:
echo [%_sid%]
echo 无法寻找到用户账户SID，正在终止
echo:
echo 请翻阅该页面并获取帮助. %mas%IAS-Help#troubleshoot
goto done2
)
...
echo 在%CLSID2%中写入失败
echo:
echo 请翻阅此页面并获取帮助. %mas%IAS-Help#troubleshoot
goto done2
)
...
echo:                              QQ群335805261
echo:                部分行列不支持中文ANSL字符，导致无法翻译，请见谅
echo:        本人不对本文件造成的任何影响负责，仅作为学习用途，进行了部分翻译
...
echo:
echo 在 %SystemRoot%\Temp中创建注册表值备份文件
...
echo 你可以在https://www.internetdownloadmanager.com/download.html下载软件
goto done
)
...
call :_color %Red% "无法连接internetdownloadmanager.com, 正在终止..."
goto done
)
call :_color %Gray% "Ping internetdownloadmanager.com 命令失败"
echo:
)
...
echo:
echo 正在在 %SystemRoot%\Temp中创建CLSID注册表值备份文件
